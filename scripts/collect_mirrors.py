...
def render_rhythm(rhythm: Dict) -> str:
    ...
    active_lanes = {k: v for k, v in rhythm["lanes"].items() if sum(v) > 0}
    pool = active_lanes if active_lanes else rhythm["lanes"]
    top_lanes = dict(
        sorted(
            pool.items(),
            key=lambda kv: (-plus.get(kv[0], 100), -sum(kv[1]), kv[0])
        )[:5]
    )
