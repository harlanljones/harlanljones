<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/header.svg" />
  <img src="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/header-light.svg" alt="Harlan Jones" width="100%" />
</picture>

<!-- MLB_BIRTHDAY_START -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/dugout-dispatch.svg" />
  <img src="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/dugout-dispatch-light.svg" alt="Daily Dugout Dispatch" width="100%" />
</picture>
<!-- MLB_BIRTHDAY_END -->

<!-- WEEKLY_HIGHLIGHTS_START -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/weekly-highlights.svg" />
  <img src="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/weekly-highlights-light.svg" alt="What I Did This Week" width="100%" />
</picture>
<!-- WEEKLY_HIGHLIGHTS_END -->

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/repo-leaderboard.svg" />
  <img src="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/repo-leaderboard-light.svg" alt="Repo Leaderboard" width="100%" />
</picture>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/projects.svg" />
  <img src="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/projects-light.svg" alt="Projects" width="100%" />
</picture>
<!-- PROJECTS_END -->

<!-- FEATURED_REPOS_START -->
<details>
  <summary><b>🚀 Featured Repositories & Live Demos</b></summary>

| Project | Focus | Tech Stack | Live Deployment |
| :--- | :--- | :--- | :--- |
| [**urban-signal**](https://github.com/harlanljones/urban-signal) | Real-time spatio-temporal property forecasting & telemetry | `Python` `Kafka` `FastAPI` `PostGIS` | [urban-signal.harlanljones.com ↗](https://urban-signal.harlanljones.com) |
| [**statcast-lakehouse**](https://github.com/harlanljones/statcast-lakehouse) | Statcast pitch telemetry ingestion, 3-yr retention & 500-pitch scenarios | `Python` `TypeScript` | [GitHub Repository ↗](https://github.com/harlanljones/statcast-lakehouse) |
| [**sabr-jev**](https://github.com/harlanljones/sabr-jev) | Sabermetric valuation engine, historical backtests & interactive cards | `Python` | [sabr.harlanljones.com ↗](https://sabr.harlanljones.com) |
| [**omarchy-agents**](https://github.com/harlanljones/omarchy-agents) | AI coding-agent usage tracking, token correlation & desktop widgets | `TypeScript` `Bun` `React` `QML` | [agents.harlanljones.com ↗](https://agents.harlanljones.com) |
| [**baseball-dashboard**](https://github.com/harlanljones/baseball-dashboard) | Live MLB scoreboard, sabermetric matchup splits & bullpen workload | `TypeScript` `Next.js` `React` `Tailwind` | [mlb.harlanljones.com ↗](https://mlb.harlanljones.com/) |
| [**arbkit**](https://github.com/harlanljones/arbkit) | Cross-venue sports & prediction market arbitrage engine (p99 < 250ns) | `Rust` `Performance` `Architecture` `Markets` | [arbkit.harlanljones.com ↗](https://arbkit.harlanljones.com/) |
| [**bayes-horizon**](https://github.com/harlanljones/bayes-horizon) | S&P 500 Bayesian macroeconomic ML forecasting & walk-forward testing | `Python` `PyMC` `FastAPI` `Cloudflare` | [sp500.harlanljones.com ↗](https://sp500.harlanljones.com/) |
| [**dotfiles**](https://github.com/harlanljones/dotfiles) | Automated developer workspace, Chezmoi configs & agent tooling | `Go Templates` `Shell` | [GitHub Repository ↗](https://github.com/harlanljones/dotfiles) |
| [**ferrite-db**](https://github.com/harlanljones/ferrite-db) | Embedded Rust approximate nearest-neighbor vector search library | `Rust` `TypeScript` | [ferritedb.harlanljones.com ↗](https://ferritedb.harlanljones.com/) |
| [**scheme-db**](https://github.com/harlanljones/scheme-db) | Interactive NFL scheme engineering & 80-play animation workstation | `TypeScript` `React` `Tailwind` `Vite` | [schemedb.harlanljones.com ↗](https://schemedb.harlanljones.com/) |
| [**film-lab**](https://github.com/harlanljones/film-lab) | Automated darkroom simulation & photo-processing pipeline | `TypeScript` `JavaScript` | [film-lab.harlanljones.com ↗](https://film-lab.harlanljones.com/) |
| [**seismic-ml**](https://github.com/harlanljones/seismic-ml) | Spatiotemporal ML pipeline for short-term USGS earthquake forecasting | `Python` `TensorFlow` `CUDA` `USGS` | [GitHub Repository ↗](https://github.com/harlanljones/seismic-ml) |
| [**herdr-outpost**](https://github.com/harlanljones/herdr-outpost) | Remote mTLS dashboard & relay gateway for AI coding agent workspaces | `Python` `Cloudflare` `Cloudflare Tunnel` `WebSocket` | [GitHub Repository ↗](https://github.com/harlanljones/herdr-outpost) |
| [**march-madness**](https://github.com/harlanljones/march-madness) | PyTorch deep learning tournament prediction & bracket simulation | `Python` `PyTorch` `Monte Carlo` `Kaggle` | [GitHub Repository ↗](https://github.com/harlanljones/march-madness) |
| [**saberkit**](https://github.com/harlanljones/saberkit) | High-performance Rust sabermetric valuation & statistics engine | `Python` `Rust` | [GitHub Repository ↗](https://github.com/harlanljones/saberkit) |

</details>
<!-- FEATURED_REPOS_END -->

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/skills.svg" />
  <img src="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/skills-light.svg" alt="Skills in Practice" width="100%" />
</picture>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/dev-grid.svg" />
  <img src="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/dev-grid-light.svg" alt="Dev Immaculate Grid" width="100%" />
</picture>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/statline.svg" />
  <img src="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/statline-light.svg" alt="Season Stat Line" width="100%" />
</picture>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/activity.svg" />
  <img src="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/activity-light.svg" alt="GitHub Activity" width="100%" />
</picture>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/commit-rhythm.svg" />
  <img src="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/commit-rhythm-light.svg" alt="Languages & Commit Rhythm" width="100%" />
</picture>

<details>
  <summary><b>📐 Sabermetric Glossary & Architecture Details</b></summary>
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/pipeline.svg" />
    <img src="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/pipeline-light.svg" alt="How This Page Is Built" width="100%" />
  </picture>

  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/glossary.svg" />
    <img src="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/glossary-light.svg" alt="Glossary" width="100%" />
  </picture>
</details>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/footer.svg" />
  <img src="https://raw.githubusercontent.com/harlanljones/harlanljones/profile-cards/footer-light.svg" alt="Footer" width="100%" />
</picture>
